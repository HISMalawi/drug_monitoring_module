  ...
  def self.find_significant_disp_pres_diff(start_date ,end_date)
    ...
    (prescriptions || []).each do |prescription|
      ...
      if dispensation.blank?
        issues << "#{prescription.site.name} has prescriptions for #{prescription.get_short_form} but no dispensations on #{prescription.value_date.strftime('%d %B %Y')}"
      else
        percent = (((prescription.value_numeric.to_f - dispensation[0].value_numeric.to_f)/prescription.value_numeric.to_f)*100).round(2)
        if percent >= 20.0
          issues << "#{prescription.site.name} has #{percent.abs}% more prescriptions than dispensations for #{prescription.get_short_form} on #{prescription.value_date.strftime('%d %B %Y')}"
        elsif percent <= -20.0
          issues << "#{prescription.site.name} has #{percent.abs}% more dispensations than prescriptions for #{prescription.get_short_form} on #{prescription.value_date.strftime('%d %B %Y')}"
        end
  ...
  def self.find_dispensation_without_prescriptions(start_date ,end_date)
    ...
    (dispensations || []).each do |dispensation|
      ...
      if prescription.blank?
        issues << "#{dispensation.site.name} has dispensation for #{dispensation.get_short_form} but no prescriptions on #{dispensation.value_date.strftime('%d %B %Y')}"
      end
    end
